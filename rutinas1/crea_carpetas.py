...
sql="SELECT sedes.nombre_sede, asignaturas.programa, asignaturas.cod_asig, secciones.seccion , nombre_doc, apellidos_doc FROM secciones join asignaturas on secciones.cod_asig = asignaturas.cod_asig join sedes on sedes.id_sede = secciones.id_sede join docentes on docentes.rut_docente = secciones.rut_docente WHERE secciones.cod_asig LIKE 'PLC11__' order by nombre_sede, programa , cod_asig;"
df=ejecutasql(sql)
i=1
for row in df.itertuples():
 #   print(row.id_seccion)
    print(row)
    nueva_carpeta = "C:/sudcraultra/carpetas_para_sedes"
    #ruta_completa = f"{nueva_carpeta}/Test de Competencias/{row.programa}/{row.nombre_sede}/{row.cod_asig}/{row.apellidos_doc} {row.nombre_doc}/{row.seccion}"
